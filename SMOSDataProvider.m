classdef SMOSDataProvider < handle
 ...
 properties
    ...
    dbRecordColumns = {'date_time', 'grid_point_id', 'observ_date', 'polarization', 'bt_real', 'bt_imag', ...
 ...
 methods
     ...
     function Status = UpdateDBGeometryTable(dProvider)
         ...
         for csvIdx=1:3%length(CSVFiles)
            % record counter
            recordCnt = 0;
            ...
            dProvider.writeDBLog(['File ' csvFileName ' is processing.']);
            
            % because we read from column const.CSV_ID_COL, not from first
            % one
            tmpColumnIdx = const.CSV_ID_COL -1;
            
            csv = dlmread(csvFileFullName,';',1,tmpColumnIdx);
            ...
            for rowIdx=1:size(csv,1)
                ...
                tId = csv(rowIdx,const.CSV_ID_COL-tmpColumnIdx);                
                
                if ~isequal(lastSMOSPointID, tId);
                    lastSMOSPointID = tId;
                    tLat = csv(rowIdx,const.CSV_LAT_COL-tmpColumnIdx);
                    tLon = csv(rowIdx,const.CSV_LON_COL-tmpColumnIdx);
     ...
     function [P_IA, P_BT] = GetIABT(dProvider, pointId, dateNum, polarization)
         ...
         sql = ['SELECT incidence_angle, bt_real FROM ' dProvider.tableRecordName ...
            ' smos_records WHERE grid_point_id = ' num2str(pointId) ' AND observ_date BETWEEN ''' ...
            datestr(dateNum, 'yyyy-mm-dd') ' 00:00:00'' AND ''' datestr(dateNum, 'yyyy-mm-dd') ' 23:59:59'' AND polarization = ' num2str(polarization) ...
            ' ORDER BY incidence_angle'];
